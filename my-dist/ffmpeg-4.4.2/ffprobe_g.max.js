...
function UTF8ArrayToString(heapOrArray, idx, maxBytesToRead) {
    ...
    if (endPtr - idx > 16 && heapOrArray.buffer && UTF8Decoder) {
        return UTF8Decoder.decode(typeof SharedArrayBuffer !== "undefined" && heapOrArray.buffer instanceof SharedArrayBuffer ? heapOrArray.slice(idx, endPtr) : heapOrArray.subarray(idx, endPtr))
    } else {
        var str = "";
        while (idx < endPtr) {
...
if (ENVIRONMENT_IS_PTHREAD) {
    ...
    if (Module["wasmMemory"]) {
        ...
        if (typeof SharedArrayBuffer !== "undefined") {
          ...
          console.warn("wasmMemory might be expected to be of a SharedArrayBuffer", wasmMemory);
        }
    }
}
if (wasmMemory) {
...
function createWasm() {
    ...
    function receiveInstance(instance, module) {
        ...
        Module["asm"] = exports;
        registerTlsInit(Module["asm"]["se"]);
        wasmTable = Module["asm"]["le"];
        addOnInit(Module["asm"]["je"]);
        wasmModule = module;
        if (!ENVIRONMENT_IS_PTHREAD) {
...
var PThread = {
    ...
    allocateUnusedWorker: function() {
        var pthreadMainJs = locateFile("ffprobe_g.worker.js");
...
function ___syscall_newfstatat(dirfd, path, buf, flags) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(17, 1, dirfd, path, buf, flags);
    try {
        path = SYSCALLS.getStr(path);
...
function ___syscall_openat(dirfd, path, flags, varargs) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(18, 1, dirfd, path, flags, varargs);
    SYSCALLS.varargs = varargs;
    try {
        path = SYSCALLS.getStr(path);
        path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_poll(fds, nfds, timeout) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(19, 1, fds, nfds, timeout);
    try {
        var nonzero = 0;
...
function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(20, 1, fd, buf, len, flags, addr, addrlen);
    try {
        var sock = getSocketFromFD(fd);
...
function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(21, 1, olddirfd, oldpath, newdirfd, newpath);
    try {
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(23, 1, fd, message, length, flags, addr, addr_len);
    try {
        var sock = getSocketFromFD(fd);
...
function ___syscall_socket(domain, type, protocol) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(24, 1, domain, type, protocol);
    try {
...
function ___syscall_stat64(path, buf) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(25, 1, path, buf);
    try {
        path = SYSCALLS.getStr(path);
...
function __mmap_js(addr, len, prot, flags, fd, off, allocated, builtin) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(27, 1, addr, len, prot, flags, fd, off, allocated, builtin);
    try {
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(28, 1, addr, len, prot, flags, fd, offset);
    try {
        var stream = FS.getStream(fd);
        if (stream) {
            if (prot & 2) {
...
function _tzset_impl(timezone, daylight, tzname) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(29, 1, timezone, daylight, tzname);
...
function _eglBindAPI(api) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(30, 1, api);
    if (api == 12448) {
        EGL.setErrorCode(12288);
        return 1
    } else {
...
function _eglChooseConfig(display, attrib_list, configs, config_size, numConfigs) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(31, 1, display, attrib_list, configs, config_size, numConfigs);
...
function _eglCreateContext(display, config, hmm, contextAttribs) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(32, 1, display, config, hmm, contextAttribs);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglCreateWindowSurface(display, config, win, attrib_list) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(33, 1, display, config, win, attrib_list);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglDestroyContext(display, context) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(34, 1, display, context);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglDestroySurface(display, surface) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(35, 1, display, surface);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglGetConfigAttrib(display, config, attribute, value) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(36, 1, display, config, attribute, value);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglGetDisplay(nativeDisplayType) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(37, 1, nativeDisplayType);
    EGL.setErrorCode(12288);
    return 62e3
}
...
function _eglInitialize(display, majorVersion, minorVersion) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(39, 1, display, majorVersion, minorVersion);
    if (display == 62e3) {
        if (majorVersion) {
...
function _eglMakeCurrent(display, draw, read, context) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(40, 1, display, draw, read, context);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglQueryString(display, name) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(41, 1, display, name);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglSwapInterval(display, interval) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(43, 1, display, interval);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglTerminate(display) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(44, 1, display);
    if (display != 62e3) {
        EGL.setErrorCode(12296);
        return 0
    }
...
function _eglWaitNative(nativeEngineId) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(46, 1, nativeEngineId);
    EGL.setErrorCode(12288);
    return 1
}
...
function _emscripten_set_canvas_element_size_main_thread(target, width, height) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(49, 1, target, width, height);
...
function _emscripten_get_element_css_size(target, width, height) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(52, 1, target, width, height);
...
function _emscripten_get_gamepad_status(index, gamepadState) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(53, 1, index, gamepadState);
...
function _emscripten_get_screen_size(width, height) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(55, 1, width, height);
...
function _emscripten_request_fullscreen_strategy(target, deferUntilInEventHandler, fullscreenStrategy) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(56, 1, target, deferUntilInEventHandler, fullscreenStrategy);
    var strategy = {
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(57, 1, target, deferUntilInEventHandler);
    target = findEventTarget(target);
    if (!target) return -4;
...
function _emscripten_set_beforeunload_callback_on_thread(userData, callbackfunc, targetThread) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(59, 1, userData, callbackfunc, targetThread);
...
function _emscripten_set_gamepaddisconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(65, 1, userData, useCapture, callbackfunc, targetThread);
    if (!navigator.getGamepads && !navigator.webkitGetGamepads) return -1;
...
function _emscripten_set_touchend_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(77, 1, target, userData, useCapture, callbackfunc, targetThread);
    registerTouchEventCallback(target, userData, useCapture, callbackfunc, 23, "touchend", targetThread);
    return 0
}

function _emscripten_set_touchmove_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(78, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_visibilitychange_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(80, 1, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_window_title(title) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(82, 1, title);
...
function _environ_get(__environ, environ_buf) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(83, 1, __environ, environ_buf);
    var bufSize = 0;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(84, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(85, 1, fd);
    try {
        var stream = SYSCALLS.getStreamFromFD(fd);
        FS.close(stream);
        return 0
...
function _fd_fdstat_get(fd, pbuf) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(86, 1, fd, pbuf);
    try {
        var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(88, 1, fd, offset_low, offset_high, whence, newOffset);
    try {
        var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_write(fd, iov, iovcnt, pnum) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(89, 1, fd, iov, iovcnt, pnum);
    try {
        var stream = SYSCALLS.getStreamFromFD(fd);
...
function _getaddrinfo(node, service, hint, out) {
    if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(90, 1, node, service, hint, out);
    var addr = 0;
    var port = 0;
    var flags = 0;
    var family = 0;
...
var asmLibraryArg = {
    "h": ___assert_fail,
    "lb": ___call_sighandler,
    "vb": ___emscripten_init_main_thread_js,
    "S": ___emscripten_thread_cleanup,
    "mb": ___pthread_create_js,
    "cb": ___syscall_accept4,
    "bb": ___syscall_bind,
    "ab": ___syscall_connect,
    "Nb": ___syscall_faccessat,
    "e": ___syscall_fcntl64,
    "Fb": ___syscall_fstat64,
    "kb": ___syscall_getdents64,
    "$a": ___syscall_getpeername,
    "Za": ___syscall_getsockname,
    "Ya": ___syscall_getsockopt,
    "Ob": ___syscall_ioctl,
    "Xa": ___syscall_listen,
    ...
    "ib": ___syscall_renameat,
    "hb": ___syscall_rmdir,
    "Va": ___syscall_sendto,
    "R": ___syscall_socket,
    "Eb": ___syscall_stat64,
    "eb": ___syscall_unlinkat,
    "m": __emscripten_date_now,
    "nb": __emscripten_default_pthread_stack_size,
    "Hb": __emscripten_get_now_is_monotonic,
    "pb": __emscripten_notify_proxying_queue,
    "xb": __emscripten_set_offscreencanvas_size,
    "Ib": __gmtime_js,
    "Jb": __localtime_js,
    "Kb": __mktime_js,
    "rb": __mmap_js,
    ...
    "wc": _eglBindAPI,
    "ld": _eglChooseConfig,
    "Ta": _eglCreateContext,
    "jb": _eglCreateWindowSurface,
    "_a": _eglDestroyContext,
    "sb": _eglDestroySurface,
    "wd": _eglGetConfigAttrib,
    "X": _eglGetDisplay,
    "Sa": _eglGetError,
    "Rc": _eglInitialize,
    "Db": _eglMakeCurrent,
    ...
    "Ma": _emscripten_exit_fullscreen,
    "Pa": _emscripten_exit_pointerlock,
    "o": _emscripten_get_device_pixel_ratio,
    "k": _emscripten_get_element_css_size,
    "u": _emscripten_get_gamepad_status,
    "fb": _emscripten_get_heap_max,
    "f": _emscripten_get_now,
    "sa": _emscripten_get_num_gamepads,
    "Qa": _emscripten_get_screen_size,
    "ra": _emscripten_glActiveTexture,
    "qa": _emscripten_glAttachShader,
    "Ia": _emscripten_glBeginQueryEXT,
    ...
    "Aa": _emscripten_glBindVertexArrayOES,
    ...
    "ce": _emscripten_glCreateShader,
    "be": _emscripten_glCullFace,
    "ae": _emscripten_glDeleteBuffers,
    "$d": _emscripten_glDeleteFramebuffers,
    "_d": _emscripten_glDeleteProgram,
    ...
    "La": _emscripten_glGenQueriesEXT,
    ...
    "zd": _emscripten_glGetAttribLocation,
    "yd": _emscripten_glGetBooleanv,
    "xd": _emscripten_glGetBufferParameteriv,
    "vd": _emscripten_glGetError,
    "ud": _emscripten_glGetFloatv,
    "td": _emscripten_glGetFramebufferAttachmentParameteriv,
    "sd": _emscripten_glGetIntegerv,
    "qd": _emscripten_glGetProgramInfoLog,
    "rd": _emscripten_glGetProgramiv,
    "Ca": _emscripten_glGetQueryObjecti64vEXT,
    "Ea": _emscripten_glGetQueryObjectivEXT,
    "Ba": _emscripten_glGetQueryObjectui64vEXT,
    "Da": _emscripten_glGetQueryObjectuivEXT,
    "Fa": _emscripten_glGetQueryivEXT,
    "pd": _emscripten_glGetRenderbufferParameteriv,
    "nd": _emscripten_glGetShaderInfoLog,
    "md": _emscripten_glGetShaderPrecisionFormat,
    "kd": _emscripten_glGetShaderSource,
    "od": _emscripten_glGetShaderiv,
    ...
    "Ga": _emscripten_glQueryCounterEXT,
    ...
    "zc": _emscripten_glTexParameterfv,
    "yc": _emscripten_glTexParameteri,
    "xc": _emscripten_glTexParameteriv,
    "vc": _emscripten_glTexSubImage2D,
    "uc": _emscripten_glUniform1f,
    "tc": _emscripten_glUniform1fv,
    "sc": _emscripten_glUniform1i,
    "rc": _emscripten_glUniform1iv,
    "qc": _emscripten_glUniform2f,
    "pc": _emscripten_glUniform2fv,
    "oc": _emscripten_glUniform2i,
    "nc": _emscripten_glUniform2iv,
    "mc": _emscripten_glUniform3f,
    "kc": _emscripten_glUniform3fv,
    "jc": _emscripten_glUniform3i,
    "ic": _emscripten_glUniform3iv,
    "hc": _emscripten_glUniform4f,
    "gc": _emscripten_glUniform4fv,
    "fc": _emscripten_glUniform4i,
    "ec": _emscripten_glUniform4iv,
    "dc": _emscripten_glUniformMatrix2fv,
    "cc": _emscripten_glUniformMatrix3fv,
    "bc": _emscripten_glUniformMatrix4fv,
    "$b": _emscripten_glUseProgram,
    "_b": _emscripten_glValidateProgram,
    "Zb": _emscripten_glVertexAttrib1f,
    "Yb": _emscripten_glVertexAttrib1fv,
    "Xb": _emscripten_glVertexAttrib2f,
    "Wb": _emscripten_glVertexAttrib2fv,
    "Vb": _emscripten_glVertexAttrib3f,
    "Ub": _emscripten_glVertexAttrib3fv,
    "Tb": _emscripten_glVertexAttrib4f,
    "Sb": _emscripten_glVertexAttrib4fv,
    "ta": _emscripten_glVertexAttribDivisorANGLE,
    "Qb": _emscripten_glVertexAttribPointer,
    "Pb": _emscripten_glViewport,
    "s": _emscripten_has_asyncify,
    "Gb": _emscripten_memcpy_big,
    "gb": _emscripten_num_logical_cores,
    "wb": _emscripten_receive_on_main_thread_js,
    "Na": _emscripten_request_fullscreen_strategy,
    ...
    "A": _emscripten_set_keypress_callback_on_thread,
    "B": _emscripten_set_keyup_callback_on_thread,
    "O": _emscripten_set_mousedown_callback_on_thread,
    "M": _emscripten_set_mouseenter_callback_on_thread,
    "L": _emscripten_set_mouseleave_callback_on_thread,
    "P": _emscripten_set_mousemove_callback_on_thread,
    ...
    "E": _emscripten_set_touchcancel_callback_on_thread,
    "G": _emscripten_set_touchend_callback_on_thread,
    "F": _emscripten_set_touchmove_callback_on_thread,
    "H": _emscripten_set_touchstart_callback_on_thread,
    "x": _emscripten_set_visibilitychange_callback_on_thread,
    "K": _emscripten_set_wheel_callback_on_thread,
    "Oa": _emscripten_set_window_title,
    "r": _emscripten_sleep,
...
var ___wasm_call_ctors = Module["___wasm_call_ctors"] = function() {
    return (___wasm_call_ctors = Module["___wasm_call_ctors"] = Module["asm"]["je"]).apply(null, arguments)
};
var ___errno_location = Module["___errno_location"] = function() {
    return (___errno_location = Module["___errno_location"] = Module["asm"]["ke"]).apply(null, arguments)
};
var _main = Module["_main"] = function() {
    return (_main = Module["_main"] = Module["asm"]["me"]).apply(null, arguments)
};
var _ntohs = Module["_ntohs"] = function() {
    return (_ntohs = Module["_ntohs"] = Module["asm"]["ne"]).apply(null, arguments)
};
var _htons = Module["_htons"] = function() {
    return (_htons = Module["_htons"] = Module["asm"]["oe"]).apply(null, arguments)
};
var _free = Module["_free"] = function() {
    return (_free = Module["_free"] = Module["asm"]["pe"]).apply(null, arguments)
};
var _malloc = Module["_malloc"] = function() {
    return (_malloc = Module["_malloc"] = Module["asm"]["qe"]).apply(null, arguments)
};
var _pthread_self = Module["_pthread_self"] = function() {
    return (_pthread_self = Module["_pthread_self"] = Module["asm"]["re"]).apply(null, arguments)
};
var _emscripten_tls_init = Module["_emscripten_tls_init"] = function() {
    return (_emscripten_tls_init = Module["_emscripten_tls_init"] = Module["asm"]["se"]).apply(null, arguments)
};
var _emscripten_builtin_memalign = Module["_emscripten_builtin_memalign"] = function() {
    return (_emscripten_builtin_memalign = Module["_emscripten_builtin_memalign"] = Module["asm"]["te"]).apply(null, arguments)
};
var __emscripten_thread_init = Module["__emscripten_thread_init"] = function() {
    return (__emscripten_thread_init = Module["__emscripten_thread_init"] = Module["asm"]["ue"]).apply(null, arguments)
};
var __emscripten_thread_crashed = Module["__emscripten_thread_crashed"] = function() {
    return (__emscripten_thread_crashed = Module["__emscripten_thread_crashed"] = Module["asm"]["ve"]).apply(null, arguments)
};
var _htonl = Module["_htonl"] = function() {
    return (_htonl = Module["_htonl"] = Module["asm"]["we"]).apply(null, arguments)
};
var _emscripten_proxy_execute_queue = Module["_emscripten_proxy_execute_queue"] = function() {
    return (_emscripten_proxy_execute_queue = Module["_emscripten_proxy_execute_queue"] = Module["asm"]["xe"]).apply(null, arguments)
};
var _emscripten_main_browser_thread_id = Module["_emscripten_main_browser_thread_id"] = function() {
    return (_emscripten_main_browser_thread_id = Module["_emscripten_main_browser_thread_id"] = Module["asm"]["ye"]).apply(null, arguments)
};
var _emscripten_run_in_main_runtime_thread_js = Module["_emscripten_run_in_main_runtime_thread_js"] = function() {
    return (_emscripten_run_in_main_runtime_thread_js = Module["_emscripten_run_in_main_runtime_thread_js"] = Module["asm"]["ze"]).apply(null, arguments)
};
var _emscripten_dispatch_to_thread_ = Module["_emscripten_dispatch_to_thread_"] = function() {
    return (_emscripten_dispatch_to_thread_ = Module["_emscripten_dispatch_to_thread_"] = Module["asm"]["Ae"]).apply(null, arguments)
};
var __emscripten_thread_free_data = Module["__emscripten_thread_free_data"] = function() {
    return (__emscripten_thread_free_data = Module["__emscripten_thread_free_data"] = Module["asm"]["Be"]).apply(null, arguments)
};
var __emscripten_thread_exit = Module["__emscripten_thread_exit"] = function() {
    return (__emscripten_thread_exit = Module["__emscripten_thread_exit"] = Module["asm"]["Ce"]).apply(null, arguments)
};
var _emscripten_stack_set_limits = Module["_emscripten_stack_set_limits"] = function() {
    return (_emscripten_stack_set_limits = Module["_emscripten_stack_set_limits"] = Module["asm"]["De"]).apply(null, arguments)
};
...
var dynCall_viij = Module["dynCall_viij"] = function() {
    return (dynCall_viij = Module["dynCall_viij"] = Module["asm"]["He"]).apply(null, arguments)
};
var dynCall_viiiiijji = Module["dynCall_viiiiijji"] = function() {
    return (dynCall_viiiiijji = Module["dynCall_viiiiijji"] = Module["asm"]["Ie"]).apply(null, arguments)
};
var dynCall_viiiji = Module["dynCall_viiiji"] = function() {
    return (dynCall_viiiji = Module["dynCall_viiiji"] = Module["asm"]["Je"]).apply(null, arguments)
};
var dynCall_jiiiii = Module["dynCall_jiiiii"] = function() {
    return (dynCall_jiiiii = Module["dynCall_jiiiii"] = Module["asm"]["Ke"]).apply(null, arguments)
};
var dynCall_jiii = Module["dynCall_jiii"] = function() {
    return (dynCall_jiii = Module["dynCall_jiii"] = Module["asm"]["Le"]).apply(null, arguments)
};
var dynCall_jiiiiii = Module["dynCall_jiiiiii"] = function() {
    return (dynCall_jiiiiii = Module["dynCall_jiiiiii"] = Module["asm"]["Me"]).apply(null, arguments)
};
var dynCall_iiiji = Module["dynCall_iiiji"] = function() {
    return (dynCall_iiiji = Module["dynCall_iiiji"] = Module["asm"]["Ne"]).apply(null, arguments)
};
var dynCall_iiijjji = Module["dynCall_iiijjji"] = function() {
    return (dynCall_iiijjji = Module["dynCall_iiijjji"] = Module["asm"]["Oe"]).apply(null, arguments)
};
var dynCall_jiiij = Module["dynCall_jiiij"] = function() {
    return (dynCall_jiiij = Module["dynCall_jiiij"] = Module["asm"]["Pe"]).apply(null, arguments)
};
var dynCall_jiji = Module["dynCall_jiji"] = function() {
    return (dynCall_jiji = Module["dynCall_jiji"] = Module["asm"]["Qe"]).apply(null, arguments)
};
var dynCall_iiiiij = Module["dynCall_iiiiij"] = function() {
    return (dynCall_iiiiij = Module["dynCall_iiiiij"] = Module["asm"]["Re"]).apply(null, arguments)
};
var dynCall_iiiiiij = Module["dynCall_iiiiiij"] = function() {
    return (dynCall_iiiiiij = Module["dynCall_iiiiiij"] = Module["asm"]["Se"]).apply(null, arguments)
};
var dynCall_jiiji = Module["dynCall_jiiji"] = function() {
    return (dynCall_jiiji = Module["dynCall_jiiji"] = Module["asm"]["Te"]).apply(null, arguments)
};
var dynCall_ijiii = Module["dynCall_ijiii"] = function() {
    return (dynCall_ijiii = Module["dynCall_ijiii"] = Module["asm"]["Ue"]).apply(null, arguments)
};
var dynCall_viiijj = Module["dynCall_viiijj"] = function() {
    return (dynCall_viiijj = Module["dynCall_viiijj"] = Module["asm"]["Ve"]).apply(null, arguments)
};
var dynCall_jij = Module["dynCall_jij"] = function() {
    return (dynCall_jij = Module["dynCall_jij"] = Module["asm"]["We"]).apply(null, arguments)
};
var dynCall_jii = Module["dynCall_jii"] = function() {
    return (dynCall_jii = Module["dynCall_jii"] = Module["asm"]["Xe"]).apply(null, arguments)
};
var dynCall_ji = Module["dynCall_ji"] = function() {
    return (dynCall_ji = Module["dynCall_ji"] = Module["asm"]["Ye"]).apply(null, arguments)
};
var _ff_h264_cabac_tables = Module["_ff_h264_cabac_tables"] = 1656924;
var __emscripten_allow_main_runtime_queued_calls = Module["__emscripten_allow_main_runtime_queued_calls"] = 3649200;
...
function callMainWithArgs(file) { 
  // https://emscripten.org/docs/api_reference/Filesystem-API.html?highlight=exception

  // Note that if the file has no full-stop, it will use the last character
  ...
  function print_file(e){
    ...
    let args = [];
    // Rerunning issues: Cannot reset args
    if (ARGS_TYPE == checkSupportedCodecs) {
      args = ['-v', 'error',"-codecs"];
    } else if (ARGS_TYPE == checkSupportedContainers) {
      args = ['-v', 'error',"-formats"];
    } else if (!calledMain) {
        args = [
        '-v',
        'error',
        '-show_streams',
        '-show_format',
        // Rerunning issues: doesn't support formatting, doesn't support showing specific entries
        // '-of',
        // 'json',
        // '-show_entries',
        // 'stream=codec_type,codec_name,codec_long_name,duration',
        // '-show_entries',
        // 'format=format_name,format_long_name,duration',
        fileName
      ];
    }
    console.log("called main", args);
    // Module.run(args);
    callMain(args);
  }
